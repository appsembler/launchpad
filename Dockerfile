...
RUN mkdir /cli && \
    if [[ $SOURCE_BRANCH == "master" ]]; then \
        apk add --no-cache git python2 py2-pip python3 wget jq openssl curl nodejs build-base libffi go && \
        ...
        apk add --no-cache git python2 py2-pip python3 wget jq openssl curl nodejs build-base libffi && \
