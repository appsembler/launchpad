...
if [ "$AVL_PAIRED_CONTAINER_EXTERNAL_DOMAIN" != "" ]; then
     ...
     sed -i "s|UtilHome.csp|UtilHome.csp?IRISUsername=tech\&IRISPassword=demo|" /home/theia/.theia/settings.json
     sed -i "s|clean=1|clean=1\&IRISUsername=tech\&IRISPassword=demo|" /home/theia/.theia/settings.json
fi
